import { z } from "zod";

const EnvSchema = z.object({
  APP_NAME: z.string().min(1),
  UPS_OAUTH_URL: z.string().url(),
  UPS_API_BASE_URL: z.string().url(),
  UPS_CLIENT_ID: z.string().min(1),
  UPS_CLIENT_SECRET: z.string().min(1),
  UPS_MERCHANT_ID: z.string().min(1).optional(),
  UPS_HTTP_TIMEOUT_MS: z.coerce.number().int().positive().default(10000),
  UPS_TOKEN_EXPIRY_SKEW_MS: z.coerce
    .number()
    .int()
    .nonnegative()
    .default(30000),
  UPS_RATING_VERSION: z.string().default("v2409"),
  UPS_RATING_ADDITIONALINFO: z.string().optional(),
  UPS_TRANS_ID: z.string().optional(),
  UPS_TRANSACTION_SRC: z.string().default("testing"),
  UPS_BILLING_ACCOUNT: z.string().min(1),
  UPS_SHIPPER_NUMBER: z.string().optional(),
});

export type Env = z.infer<typeof EnvSchema>;

export function loadEnv(processEnv = process.env): Env {
  const parsed = EnvSchema.safeParse(processEnv);
  if (!parsed.success) {
    throw new Error(`Invalid environment variables: ${parsed.error.message}`);
  }
  return parsed.data;
}
