...
import { UpsTokenProvider } from "../../src/carriers/ups";
...
describe("UPS OAuth Client Credentials - UpsTokenProvider", () => {
  ...
  it("acquires token and builds correct UPS request", async () => {
    const scope = nock(BASE_URL, {
      reqheaders: {
        ...
        authorization: (value) =>
          typeof value === "string" && value.startsWith("Basic "),
      },
    })
    ...
    const provider = new UpsTokenProvider();
  ...
  it("reuses cached token when still valid", async () => {
    const scope = nock(BASE_URL).post("/security/v1/oauth/token").reply(200, {
      token_type: "Bearer",
      issued_at: "1706710921672",
      client_id: "test_client_id",
  ...
  it("refreshes token after expiry", async () => {
    ...
    try {
      const first = nock(BASE_URL).post("/security/v1/oauth/token").reply(200, {
        token_type: "Bearer",
        issued_at: "1706710921672",
        client_id: "test_client_id",
  ...
  it("throws CARRIER_ERROR for malformed UPS response", async () => {
    nock(BASE_URL).post("/security/v1/oauth/token").reply(200, {
      token_type: "Bearer",
    });
